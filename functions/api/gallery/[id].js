export async function onRequest(context) {
  ...
  console.log("api/gallery", method, id);
  ...
  if (method === "GET") {
    const { results } = await db
      .prepare("SELECT * FROM bens_bikes_gallery WHERE id = ?")
      .bind(id)
      .all();
  ...
  if (method === "DELETE") {
    await db
      .prepare("DELETE FROM bens_bikes_gallery WHERE id = ?")
      .bind(id)
      .run();
